...
import { getCartItemById } from '../../../hooks/utils/cartUtils';
import { getMaxDiscount, getRemainingStock } from '../../../hooks/utils/productUtils';
...
export const ProductItemCard = ({ product, cart, onClick }: IProductItemCardProps) => {
    ...
    <div data-testid={`product-${product.id}`} className="bg-white p-3 rounded shadow">
      ...
      <div className="text-sm text-gray-500 mb-2">
        ...
        {product.discounts.length > 0 && (
          <span className="ml-2 font-medium text-blue-600">최대 {maxDiscount}% 할인</span>
        )}
      </div>
      {product.discounts.length > 0 && (
